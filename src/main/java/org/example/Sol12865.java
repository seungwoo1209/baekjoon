...
//        list.forEach(stuff -> System.out.println(stuff.weight + " " + stuff.value));
        ...
        for(int i=0;i<N;i++){
            ...
            HashMap<Integer,Integer> oldBag = (HashMap<Integer, Integer>) bag.clone();
            oldBag.forEach((weight,value) -> {
                int newWeight = weight + stuff.weight;
                if(newWeight <= maxWeight){
                    if(oldBag.get(newWeight) != null){
                        if(oldBag.get(newWeight) < oldBag.get(weight) + stuff.value){
                            bag.put(newWeight, oldBag.get(weight) + stuff.value);
                        }
                    } else {
                        bag.put(newWeight, oldBag.get(weight) + stuff.value);
                    }
            ...
            if(stuff.weight <= maxWeight){
                if(bag.get(stuff.weight) == null){
                    bag.put(stuff.weight, stuff.value);
                } else {
                    if(bag.get(stuff.weight) < stuff.value) bag.put(stuff.weight, stuff.value);
                }
            }
